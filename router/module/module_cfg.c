...
static const char *keys[] = {"register_addr", "server_ip","server_port", "router_node_name","moon_node_max_count","log_level_debug","log_level_info","log_level_warnning","log_level_error"};/*define key*/

bool getValue(const char* keyAndValue,const char * key, router_server_config* pConf)/*get value by key*/
{
	char val[1024] = {0};
	char tmpVal[1024] = {0};
	unsigned int keyLength = moon_get_string_length(key);
	moon_trim((char*)&(keyAndValue[keyLength]),tmpVal);//
	moon_trim(&(tmpVal[1]),val);//
	if (strcmp("register_addr",key) == 0)
	{
		...
		{
			moon_write_error_log("register addr is null");
			return false;
		}
		strcpy(pConf->register_addr,val);
	}
	else if (strcmp("server_ip",key) == 0)
	{
		//
		memset(tmpVal,0,1024);
		memcpy(tmpVal,val,1024);
	...
	else if(strcmp("router_node_name",key) == 0)
	{
		//
		memset(tmpVal,0,1024);
		memcpy(tmpVal,val,1024);
		...
		if(stringIsEmpty(val))
		{
			moon_write_error_log("router node name is null");
			return false;
		}
		strcpy(pConf->router_node_name,val);
	}
	else if(strcmp("moon_node_max_count",key) == 0)
	{
		//
		memset(tmpVal,0,1024);
		memcpy(tmpVal,val,1024);
		...
		if (!moon_string_parse_to_int(val,&(pConf->moon_node_max_count)))
		{
			return false;
		}
	}
...
bool load_config(router_server_config* pConf)//load configuration file
{
	...
	moon_write_info_log(keyValue);
	if(stringIsEmpty(pConf->router_node_name))
	{
		moon_write_error_log("router_node_name is null");
		return false;
	}
	memset(keyValue,0,1024);
	sprintf(keyValue,"router_node_name:%s",pConf->router_node_name);
	moon_write_info_log(keyValue);
	memset(keyValue,0,1024);
	sprintf(keyValue,"moon_node_max_count:%d",pConf->moon_node_max_count);
	moon_write_info_log(keyValue);
	memset(keyValue,0,1024);
	sprintf(keyValue,"router_server_ip:%s",pConf->server_ip);
	moon_write_info_log(keyValue);
	memset(keyValue,0,1024);
	return bFlag;
}
