...
_global_ router_server_config* p_router_server_config;//外部全局配置

_global_ extern bool b_config_load_finish;//配置是否加载完成
...
void moon_write_info_log(char* log)
{
	...
	if (b_config_load_finish && p_router_server_config != NULL && 'Y' != p_router_server_config->log_level_info)
		return;
	if (log == NULL)
	{
		return;
...
void moon_write_error_log(char* log)
{
	...
	char* pFormateLog = NULL;
	if (b_config_load_finish && p_router_server_config != NULL && 'Y' != p_router_server_config->log_level_error)
		return;
	if (log == NULL)
	{
		return;
...
void moon_write_warn_log(char* log)
{
	...
	char* pFormateLog = NULL;
	if (b_config_load_finish && p_router_server_config != NULL && 'Y' != p_router_server_config->log_level_warnning)
		return;
	if (log == NULL)
	{
		return;
...
void moon_write_debug_log(char* log)
{
	...
	char* pFormateLog = NULL;
	if (b_config_load_finish && p_router_server_config != NULL && 'Y' != p_router_server_config->log_level_debug)
		return;
	if (log == NULL)
	{
		return;
